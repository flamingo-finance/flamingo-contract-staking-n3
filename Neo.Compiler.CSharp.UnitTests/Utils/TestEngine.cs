...
namespace Neo.Compiler.CSharp.UnitTests.Utils
{
    public class TestEngine : ApplicationEngine
    {
        ...
        public CompilationContext AddEntryScript(string[] filenames)
        {
            var a = EntryScriptHash;

            CompilationContext context = CompilationContext.Compile(filenames, references, new Options
            {
                AddressVersion = ProtocolSettings.Default.AddressVersion
            });
            if (context.Success)
